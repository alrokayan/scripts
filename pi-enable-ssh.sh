...
function ENABLE_CONFIG_SSH() {
    ...
    sed -i'' 's/#PermitRootLogin prohibit-password/PermitRootLogin yes/' "$ROOT_MNT_PATH/etc/ssh/sshd_config"
    grep "PermitRootLogin yes" "$ROOT_MNT_PATH/etc/ssh/sshd_config"
    sed -i'' 's/PasswordAuthentication no/PasswordAuthentication yes/' "$ROOT_MNT_PATH/etc/ssh/sshd_config"
    sed -i'' 's/#PasswordAuthentication yes/PasswordAuthentication yes/' "$ROOT_MNT_PATH/etc/ssh/sshd_config"
    grep "PasswordAuthentication yes" "$ROOT_MNT_PATH/etc/ssh/sshd_config"
    sed -i'' 's/#   StrictHostKeyChecking ask/    StrictHostKeyChecking no/' "$ROOT_MNT_PATH/etc/ssh/ssh_config"
    grep "StrictHostKeyChecking no" -B 14 "$ROOT_MNT_PATH/etc/ssh/ssh_config" 
    sed -i'' 's/#net.ipv4.ip_forward=1/net.ipv4.ip_forward=1/' "$ROOT_MNT_PATH/etc/sysctl.conf"
    grep "net.ipv4.ip_forward=1" "$ROOT_MNT_PATH/etc/sysctl.conf"
    sed -i'' 's/#net.ipv6.conf.all.forwarding=1/net.ipv6.conf.all.forwarding=1/' "$ROOT_MNT_PATH/etc/sysctl.conf"
    grep "net.ipv6.conf.all.forwarding=1" "$ROOT_MNT_PATH/etc/sysctl.conf"
    sed -i'' 's/#DefaultTimeoutStopSec=90s/DefaultTimeoutStopSec=10s/' "$ROOT_MNT_PATH/etc/systemd/system.conf"
